...
Menu::Menu(ALLEGRO_DISPLAY *display, ALLEGRO_FONT *font, MenuType type):
    ...
    event_queue(nullptr)
{
    ...
    color_selected = al_map_rgb(255, 255, 0); // amarelo
}
...
void Menu::draw() {
    al_draw_filled_rectangle(0, 0, 
                            al_get_display_width(main_display),
                            al_get_display_height(main_display),
                            color_background);
    float screen_width = al_get_display_width(main_display);
    float font_height = al_get_font_line_height(menu_font);
    float start_y = (al_get_display_height(main_display) - (m_options.size() * font_height * 1.5)) / 2.0;

    for (size_t i = 0; i < m_options.size(); ++i) {
        ALLEGRO_COLOR current_color = (i == selected_option) ? color_selected : color_text;
...
MenuResult Menu::show() {
    is_active = true;
    ALLEGRO_EVENT ev;
    MenuResult final_result = MenuResult::EXIT_GAME;
    while(is_active) {
