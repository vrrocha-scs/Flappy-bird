...
OBJS = $(OBJ_DIR)/main.o $(OBJ_DIR)/personagem.o $(OBJ_DIR)/objetorenderizavel.o $(OBJ_DIR)/obstaculo.o $(OBJ_DIR)/cadastro.o $(OBJ_DIR)/hitbox.o $(OBJ_DIR)/randomizador.o $(OBJ_DIR)/menu.o 
LIBS = $(shell pkg-config --cflags --libs allegro-5 allegro_main-5 allegro_audio-5 allegro_image-5 allegro_primitives-5)
all: main 
...
$(OBJ_DIR)/main.o : $(INCLUDE_DIR)/personagem.hpp $(SRC_DIR)/main.cpp $(INCLUDE_DIR)/obstaculo.hpp $(INCLUDE_DIR)/cadastro.hpp $(INCLUDE_DIR)/randomizador.hpp $(INCLUDE_DIR)/menu.hpp
...
clean: 
	rm -f main ${OBJ_DIR}/*.o bin/main.exe 